...
<div class="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
        <div class="text-center">
            <h2 class="text-3xl font-extrabold text-gray-900">Verifikasi WhatsApp Anda</h2>
            <p class="mt-2 text-sm text-gray-600">Kode verifikasi telah dikirim ke <span class="font-semibold">{{ $user->phone }}</span></p>
        </div>
    </div>
        ...
        <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
            ...
            <form method="POST" action="{{ route('verification.wa.check') }}" class="space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <button type="submit" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Verifikasi WhatsApp</button>
                </div>
            </form>
            <form method="POST" action="{{ route('verification.wa.send') }}" class="mt-4">
                @csrf
